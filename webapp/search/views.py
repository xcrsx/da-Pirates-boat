...
from webapp.search.search_in_sc import search_sc
...
def search():
    ...
    return render_template('search/search.html', title="Let's find smth", form=form)


@blueprint.route('/search_result', methods=['GET', 'POST'])
def search_result():
    list_of_songs = search_sc()
    return render_template('search/search_result.html', title="what do we have here",
